...
def setup_connection( user ):
	...
	returns imaplib.IMAP4_SS object or 1
	@params:
...
def create_file( path, pdf_bytes ):
	"""
	takes path and bytes
	creates file in existing directory
	@params:
...
def create_path( fullname, base_dir ):
	'''
	get file name from email message
	checks it for mathing mask
	return path to save file and file name
	@params:
		- fullname	- Required	: complete filename from email (Str)
		- base_dir	- Required	: directory where are all files (Str)
	'''
	p = fullname.split('_')
